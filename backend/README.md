...
| **Refresh token** (random hex) | 30 дней | SHA-256 хэш в таблице `user_refresh_tokens` |
...
4. Refresh-токен ротируется при каждом использовании (старый инвалидируется)
...
- При регистрации или запросе верификации: задача `send_code_email_task` / `send_code_phone_task`
- При генерации тренировки: уведомление через email, SMS и push на все устройства пользователя
- При запросе восстановления пароля: задача `send_code_email_task`

---

## AI-интеграция

Сервис использует **GPT-4o mini** (OpenAI) для двух задач:
...
### Генерация рекомендаций (`POST /recommendations/refresh`)

GPT-4o получает профиль пользователя (вес, рост, цель, активность) и генерирует 3–5 конкретных рекомендаций в формате JSON-массива. Каждая рекомендация содержит тип, описание и приоритет.
